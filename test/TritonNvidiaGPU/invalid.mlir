...
// Verify: ws_wait_barrier barrierIdx out of range
#shared2 = #ttg.swizzled_shared<{vec = 1, perPhase = 1, maxPhase = 1, order = [0]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  tt.func @ws_wait_barrier_bad_idx(
      %bar: !ttg.memdesc<1xi64, #shared2, #ttg.shared_memory, mutable>,
      %accum: i64) {
    ttng.subtiled_region
        barriers(%bar : !ttg.memdesc<1xi64, #shared2, #ttg.shared_memory, mutable>)
      ...
      setup {
        ...
        // expected-error @+1 {{barrierIdx 5 is out of range}}
        ttng.ws_wait_barrier {barrierIdx = 5 : i32}
        ttng.subtiled_region_yield
      } teardown {
        ttng.subtiled_region_yield
      }
...
// Verify: ws_arrive_barrier loweringMask wrong length
#shared2 = #ttg.swizzled_shared<{vec = 1, perPhase = 1, maxPhase = 1, order = [0]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  tt.func @ws_arrive_barrier_bad_mask(
      %bar: !ttg.memdesc<1xi64, #shared2, #ttg.shared_memory, mutable>,
      %accum: i64) {
    ttng.subtiled_region
        barriers(%bar : !ttg.memdesc<1xi64, #shared2, #ttg.shared_memory, mutable>)
      ...
      setup {
        ...
        // expected-error @+1 {{loweringMask has 3 entries but there are 2 tiles}}
        ttng.ws_arrive_barrier {barrierIdx = 0 : i32, count = 1 : i32, loweringMask = array<i32: 1, 0, 1>}
        ttng.subtiled_region_yield
      } teardown {
        ttng.subtiled_region_yield
      }
